// ============ 4gtv Vercel Edge v2 (诊断增强 + 双主机轮询) ============
export const config = { runtime: 'edge' };

const HOSTS = ["https://api2.4gtv.tv", "https://api.4gtv.tv"];
...
async function getPlainKey() {
  ...
  const resp = await apiFetch(HOSTS[0] + "/App/GetAPPConfig", { method: "POST", headers: HD_IOS, body: JSON.stringify({ fsDEVICE: "iOS", fsVERSION: "3.1.0" }) });
...
async function getChannels() {
  ...
  const resp = await apiFetch(HOSTS[0] + "/Channel/GetAllChannel/mobile/L", { headers: { ...HD_ANDROID, "4GTV_AUTH": await getAuth() } });
...
async function getStreamUrl(chId, asset) {
  ...
  const strategies = [
    { hd: HD_IOS, bodyType: "mobile", idType: "string" },
    { hd: HD_ANDROID, bodyType: "mobile", idType: "string" },
    { hd: { ...HD_ANDROID, fsDEVICE: "TV", fsVERSION: "1.5.4", UserAgent: undefined, "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64)" }, bodyType: "tv", idType: "string" }
  ];

  let lastErr = "";
  for (const host of HOSTS) {
    for (const s of strategies) {
      const payload = { "fsASSET_ID": asset, "fnCHANNEL_ID": s.idType === "string" ? String(chId) : Number(chId), "clsAPP_IDENTITY_VALIDATE_ARUS": { "fsVALUE": "", "fsENC_KEY": FS_ENC_KEY }, "fsDEVICE_TYPE": s.bodyType };
      try {
        const resp = await apiFetch(host + "/App/GetChannelUrl2", { method: "POST", headers: { ...s.hd, "4GTV_AUTH": await getAuth() }, body: JSON.stringify(payload) });
        const text = await resp.text();
        if (text.trim().startsWith("<") || text.includes("<html")) { lastErr = `[${host}] HTML拦截: ${text.slice(0, 120)}`; continue; }
        ...
        lastErr = `[${host}] JSON无URL Err:${j?.ErrMessage}`;
      } catch (e) { lastErr = `[${host}] 异常: ${e.message}`; }
    }
  }
...
export default async function handler(request) {
  ...
  try {
    ...
    if (u.searchParams.get("debug") === "1") {
      let out = `UTC日期: ${new Date().toISOString().slice(0, 10)}\nauth前16位: ${(await getAuth()).slice(0, 16)}\n\n`;
      try {
        const r = await apiFetch(HOSTS[0] + "/App/GetAPPConfig", { method: "POST", headers: HD_IOS, body: JSON.stringify({ fsDEVICE: "iOS", fsVERSION: "3.1.0" }) });
        const t = await r.text();
        out += `[config] ${r.status}: ${t.slice(0, 100)}\n\n`;
      } catch (e) { out += `[config] 异常: ${e.message}\n\n`; }
      try { out += `[列表] 频道数: ${(await getChannels()).length}\n\n`; } catch (e) { out += `[列表] 异常: ${e.message}\n\n`; }
      try {
        const r = await apiFetch(HOSTS[0] + "/App/GetChannelUrl2", { method: "POST", headers: { ...HD_IOS, "4GTV_AUTH": await getAuth() }, body: JSON.stringify({ fsASSET_ID: "4gtv-4gtv003", fnCHANNEL_ID: "1", clsAPP_IDENTITY_VALIDATE_ARUS: { fsVALUE: "", fsENC_KEY: FS_ENC_KEY }, fsDEVICE_TYPE: "mobile" }) });
        const t = await r.text();
        out += `[取流原始报文] ${r.status}:\n${t.slice(0, 400)}\n`;
      } catch (e) { out += `[取流] 异常: ${e.message}\n`; }
      return txt(out);
    }
    // ===== M3U =====
